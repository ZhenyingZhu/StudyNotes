...
- leap seconds messed up timing assumptions and crash large systems. smearing: let NTP adjustment the leap second gradually over the course of a day
...
- the clock on mobile or embedded devices are not trusted, because user can change the time
- for system requires extreme clock accuracy, can use GPS receivers, the Precision Time Protocol (PTP)
...
- last write wins (LWW): can use the timestamp generated by the client not the server, but still cannot solve time skew issue
